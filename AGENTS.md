...
funcionalidad nace como una tarea del tablero y recorre cuatro fases. Para tareas de
código, Codex aprueba cada puerta tras una revisión independiente:

```
/work-task  →  [revisión de Codex]  →  /spec-task  →  [revisión de Codex]  →  /implement-task  →  [reviewer]  →  /close-task
...
|------|-------|---------|---------|
| Planificar | `/work-task TASK-NN` | `docs/plans/plan-TASK-NN.md` | Aprobación de Codex según el alcance |
| Especificar | `/spec-task TASK-NN` | `docs/plans/spec-TASK-NN.md` | Aprobación de Codex según la especificación |
...
Cada skill **se detiene** al final de su fase. Codex encadena las fases de las tareas
de código tras revisar el resultado: un reviewer independiente debe inspeccionar la
implementación; si encuentra un error, se reabre la implementación y se repite la
revisión. Si algo se atasca en cualquier fase, la tarea pasa a `Bloqueada` con el
motivo anotado en `docs/tasks/STATUS.md` y en el plan.
